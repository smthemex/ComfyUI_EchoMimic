...
def Echo_v3_predata(clip_image_encoder,text_encoder,tokenizer,face_img,audio_path,ip_mask_path,config,device,
    ...
    if clip_image is not None:
        clip_image = TF.to_tensor(clip_image).sub_(0.5).div_(0.5).to(device, config.weight_dtype) #torch.Size([3, 1008, 576])
        ...
        clip_image = TF.to_tensor(clip_image).sub_(0.5).div_(0.5).to(device, config.weight_dtype) 
        #clip_context = self.clip_image_encoder([clip_image[:, None, :, :]])
