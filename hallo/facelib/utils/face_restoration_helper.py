...
from ..utils.misc import img2tensor, imwrite, is_gray, bgr2gray, adain_npy
...
class FaceRestoreHelper(object):
    ...
    def __init__(self,
                 ...
                 face_size=512,
                 crop_ratio=(1, 1),
                 det_model='./retinaface_resnet50',#path
                 save_ext='png',
        ...
        self.pars_model_path=parse_model
        self.det_model = det_model
    
        if  "dlib" in self.det_model: #dlib
        ...
        if  "dlib" in self.det_model:
            self.face_detector, self.shape_predictor_5 = self.init_dlib(dlib_model_url['face_detector'], dlib_model_url['shape_predictor_5'])
        else:
            self.face_detector = init_detection_model(self.det_model, half=False, device=self.device)

        # init face parsing model
        self.use_parse = use_parse
        self.face_parse = init_parsing_model(model_name='parsenet',pars_model_path=self.pars_model_path, device=self.device)
    ...
    def paste_faces_to_input_image(self, save_path=None, upsample_img_list=None, draw_box=False, face_upsampler=None):
        ...
        results = []
        #print(f"upsample_img_list is {len(upsample_img_list)}") #chunk number
        for i, upsample_img in enumerate(upsample_img_list):
            ...
            #print(f"Chunk {i} inference took: {time.perf_counter() - start:.4f} seconds")
