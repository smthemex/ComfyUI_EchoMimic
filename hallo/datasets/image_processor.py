...
class ImageProcessor:
    ...
    def preprocess(self, ref_image_pil:Image.Image, cache_dir: str, face_region_ratio: float,task_model_path):
        ...
        img_file_prefix = ''.join(random.choice("0123456789abcdefg") for _ in range(6))
        file_name_prefix=f"temp_{img_file_prefix}.png"
        source_image_file = os.path.join(folder_paths.get_output_directory(),file_name_prefix)
        ref_image_pil.copy().save(source_image_file)
